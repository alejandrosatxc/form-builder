...
const TemplateAnalysis = ({ Gdoc, setGdoc }) => {
    ...
    return (
        <div className="flex flex-col justify-center bg-white rounded-xl shadow-xl my-8 min-w-[550px]">
            ...
            <h1 className="text-4xl text-primary mt-2 mb-4 mx-8">{GdocData.title}</h1>
            <div className="flex flex-row justify-center w-full">
                <div className='border rounded shadow-well bg-slate-100 border-slate-300 ml-8 mr-4 p-2 h-20 w-1/2'>
                    <h2 className='text-primary text-3xl'>{GdocData.uniqueMatches.length}</h2>
                    <p className='text-slate-600'>Unique Fields Identified</p>
                </div>
                <div className='border rounded shadow-well bg-slate-100 border-slate-300 ml-4 mr-8 p-2 h-20 w-1/2'>
            ...
            <h2 onClick={() => { setFieldsToggle(!fieldsToggle) }} className={fieldsToggle ? activeTextToggle : inactiveTextToggle}>{fieldsToggle ? "Fields -" : "Fields +"}</h2>
